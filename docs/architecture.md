...
- 单条缓存上限 256MB；即使 CDN 不返回 `Content-Length`，写入过程也会在超过预算前终止，避免合并 MP4 占满旧车机存储。
...
- 本地历史逐条显示“离线可用、缓存中、缓存失败、仅在线”；Media3 进入错误或空闲状态后，再次点击播放会重新 `prepare` 当前媒体项。
- 用户界面只显示简短错误提示，完整异常和堆栈统一写入 `CarViewModel` Logcat，便于调试而不干扰驾驶界面。
