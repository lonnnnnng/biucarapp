...
播放页根据当前媒体项是否存在分 P 标题区分布局：多 P 显示左侧播放列表，列表行只显示接口返回的分 P 标题，不额外拼接序号；单 P 不显示播放列表，左侧显示资源封面。两种布局的左侧内容与右侧播放器均按相同权重分栏，单 P 封面保持正方形并裁剪填充，避免宽屏封面拉伸。右侧控制按钮保持同一横排，播放状态说明文案不放在控制区，避免占用车机屏幕空间。
